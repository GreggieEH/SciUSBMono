...
//		MultiByteToWideChar(CP_ACP, 0, szString, -1,
	//		wszValue, 40);
		//St
		hr = VarI4FromStr(szString, LOCALE_SYSTEM_DEFAULT, 0,
			pvalue);
					...
					double			*	pvalue)
{
	...
	*pvalue = defValue;
	_stprintf_s_l(szDefault, 40, TEXT("%10.5g"), locale, defValue);
	if (ReadStringValue(szSectionName, szValueName, szDefault, szValue, 16))
	{
		if (1 == _stscanf_s_l(szValue, TEXT("%f"), locale, &fval))
		{
			fSuccess = TRUE;
			*pvalue = fval;
		}
...
*/
	...
	_stprintf_s_l(szValue, 40, TEXT("%10.5g"), locale, value);
	StrTrim(szValue, TEXT(" "));
	return WriteStringValue(szSectionName, szValueName, szValue);
}
